...
import { Phone, Key, Lock, ArrowRight, Settings, Sun, Moon, HelpCircle, ExternalLink, X, Heart, ShieldCheck } from "lucide-react";
import { useTheme } from '../context/ThemeContext';
import { telegramService } from '../services/TelegramClient';

type Step = "setup" | "phone" | "code" | "password" | "vault" | "save_vault";
...
export function AuthWizard({ onLogin }: { onLogin: () => void }) {
    ...
    const saveCredentials = async () => {
        // We no longer save to localStorage for security
    };
    ...
    const handlePhoneSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await telegramService.connect(idInt, apiHash);
    ...
    const handleVaultSubmit = async (e: React.FormEvent) => {
        ...
        setError(null);
        try {
            const creds = await telegramService.loadFromVault(masterPassword);
            setApiId(String(creds.apiId));
            setApiHash(creds.apiHash);
            await telegramService.connect(creds.apiId, creds.apiHash, true);
            onLogin();
        } catch (err: any) {
    ...
    const handleSaveVaultSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const idInt = parseInt(apiId, 10);
            await telegramService.saveToVault(masterPassword, idInt, apiHash);
            onLogin();
        } catch (err: any) {
            setError(err.message || String(err));
        } finally {
        ...
        <div className="h-full w-full auth-gradient flex items-center justify-center p-6 relative">
            ...
            >
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-2xl font-bold text-white mb-1 tracking-tight">TeleCloudFS</h1>
                        ...
                        <>
                            ...
                            {step === "setup" && (
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        className="w-full bg-white/5 hover:bg-white/10 text-white font-medium py-3 rounded-xl flex items-center justify-center gap-2 transition-all border border-white/10"
                                    >
                                        Login with Master Password <Lock className="w-4 h-4" />
                                    </button>
                            ...
                            {step === "vault" && (
                                ...
                                >
                                    <div className="text-center mb-6">
                                        ...
                                        <h2 className="text-xl font-bold text-white">Vault Login</h2>
                                        <p className="text-xs text-white/50">Enter your master password to load saved session</p>
                                    </div>
                                    ...
                                    >
                                        {loading ? "Decrypting..." : "Unlock Vault"} <ArrowRight className="w-4 h-4" />
                                    </button>
                            ...
                            {step === "save_vault" && (
                                <motion.form
                                    ...
                                    onSubmit={handleSaveVaultSubmit}
                                    className="space-y-5"
                                >
                                    <div className="text-center mb-6">
                                        <ShieldCheck className="w-12 h-12 text-green-400 mx-auto mb-3" />
                                        <h2 className="text-xl font-bold text-white">Save to Cloud?</h2>
                                        <p className="text-xs text-white/50">Set a master password to avoid OTP logins in the future.</p>
                                    </div>
                                    ...
                                    <button
                                        type="submit"
                                        disabled={loading}
                                        className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                                    >
                                        {loading ? "Saving..." : "Save Session to D1"} <ArrowRight className="w-4 h-4" />
                                    </button>
                                    ...
                                    <button
                                        ...
                                        className="w-full text-xs text-gray-400 hover:text-white transition-colors py-1"
                                    >
                                        Skip (I'll login with OTP next time)
                                    </button>
                                </motion.form>
                            )}
                            ...
                            <div className="space-y-6 text-telegram-text">
                                <div className="p-4 bg-telegram-primary/10 border border-telegram-primary/20 rounded-xl">
                                    <p className="text-sm text-telegram-subtext">
                                        <strong className="text-telegram-primary">TeleCloudFS</strong> uses your Telegram account as secure cloud storage. You'll need a Telegram account and API credentials to get started.
                                    </p>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <p className="text-sm text-telegram-subtext ml-8">
                                        Visit <button type="button" onClick={(e) => { e.preventDefault(); window.open('https://my.telegram.org'); }} className="text-telegram-primary underline hover:text-telegram-text cursor-pointer">my.telegram.org</button> and log in with your phone number.
                                    </p>
                                </div>
                                ...
                                <button
                                    type="button"
                                    onClick={(e) => { e.preventDefault(); window.open('https://my.telegram.org'); }}
                        ...
                        >
                            ...
                            <div className="space-y-4 text-center">
                                <p className="text-sm text-telegram-subtext mb-6">
                                    If you find TeleCloudFS useful, consider supporting its development!
                                </p>

                                <div className="space-y-4">
                                    <a href="#" onClick={(e) => { e.preventDefault(); window.open('https://www.paypal.me/Caamer20'); }} className="block hover:opacity-80 transition-opacity">
                                    ...
                                    <a href="#" onClick={(e) => { e.preventDefault(); window.open('https://link.trustwallet.com/send?address=ltc1q6wkr5ac4u0pxx4hx7xgwn0gsaku25ws0df73rp&asset=c2'); }} className="block hover:opacity-80 transition-opacity">
                                    ...
                                    <a href="#" onClick={(e) => { e.preventDefault(); window.open('https://link.trustwallet.com/send?asset=c0&address=bc1q5pt7m2fk6w0dzsnf6vvd5k6nw5k44785286ujy'); }} className="block hover:opacity-80 transition-opacity">
