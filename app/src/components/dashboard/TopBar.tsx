...
export function TopBar({
    currentFolderName, fileCount, selectedIds, onShowMoveModal, onBulkDownload, onBulkDelete,
    onDownloadFolder, onSelectAll, viewMode, setViewMode, searchTerm, onSearchChange
}: TopBarProps) {
        ...
        <header className="h-14 border-b border-telegram-border flex items-center px-4 justify-between bg-telegram-surface/80 backdrop-blur-md sticky top-0 z-10" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-4">
                <div className="flex items-center text-sm breadcrumbs text-telegram-subtext select-none">
                    ...
                    <span className="ml-3 px-2 py-0.5 bg-telegram-primary/10 text-telegram-primary text-[10px] rounded-full font-bold">
            ...
            <div className="flex-1 max-w-md mx-4">
                <input
                    type="text"
                    placeholder='Search... (e.g. "exact" or ext:pdf)'
            ...
            <div className="flex items-center gap-2">
                {selectedIds.length > 0 && (
                    <div className="flex items-center gap-2 mr-4 animate-in fade-in slide-in-from-top-2">
                        ...
                        >
                            {selectedIds.length === fileCount ? 'Deselect All' : 'Select All'}
                        </button>
