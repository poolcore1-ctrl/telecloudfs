...
import { HardDrive, Folder, Plus, RefreshCw, LogOut, Key } from 'lucide-react';
...
interface SidebarProps {
    ...
    currentView: 'drive' | 'api-keys';
    onViewChange: (view: 'drive' | 'api-keys') => void;
}
    ...
    return (
        <aside className="w-64 bg-telegram-surface border-r border-telegram-border flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-4 flex items-center gap-2">
                ...
                <span className="font-bold text-lg text-telegram-text tracking-tight">TeleCloudFS</span>
            </div>
            ...
            <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto min-h-0">
                <SidebarItem
                    ...
                    active={currentView === 'drive' && activeFolderId === null}
                    onClick={() => {
                        onViewChange('drive');
                ...
                {folders.map(folder => (
                    <SidebarItem
                        ...
                        icon={Folder}
                        label={folder.name}
                        active={currentView === 'drive' && activeFolderId === folder.id}
                        onClick={() => {
                            onViewChange('drive');
                ...
                <div className="pt-4 pb-2 px-3 text-[10px] font-bold text-telegram-subtext uppercase tracking-wider">
                    Developer
                </div>
                <SidebarItem
                    icon={Key}
