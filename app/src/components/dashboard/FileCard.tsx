...
export function FileCard({ file, onDelete, onDownload, isSelected, onClick, onContextMenu, onDrop, onDragStart, onDragEnd, activeFolderId, height, onToggleSelection }: FileCardProps) {
    ...
    const handleShare = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            if (keys && keys.length > 0) {
                const folderPart = (activeFolderId === null || activeFolderId === undefined) ? 'home' : activeFolderId.toString();
    ...
    useEffect(() => {
        ...
        telegramService.getThumbnailUrl(file.id, activeFolderId ?? null).then((result) => {
            if (!cancelled && result) {
                setThumbnail(result);
            }
        }).catch(() => {
            ...
            >
                ...
                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 z-10">
                    <button onClick={async (e) => { 
                        e.stopPropagation(); 
                        const folderPart = (activeFolderId === null || activeFolderId === undefined) ? 'home' : activeFolderId.toString();
                        const shareUrl = `${window.location.origin}/api/s3/${folderPart}/${file.id}`;
                        window.open(shareUrl, '_blank');
                    }} className="file-action-btn p-1 bg-black/50 rounded-full hover:bg-telegram-primary hover:text-white text-white/70" title="Open in New Tab">
                    ...
                    <button onClick={handleShare} className="file-action-btn p-1 bg-black/50 rounded-full hover:bg-blue-500 hover:text-white text-white/70" title="Share">
